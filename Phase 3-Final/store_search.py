...
def check_stores(_conn,location):
    print("Inventory of the store:")
    try:
        sql = """select *
        from Store
...
def platform_catalog(_conn,develop):
    print("\nPlatform catalog:")
    try:
        if(develop == True):
            sql = """select Platform.name
            from Platform
...
def search(_conn):
    ...
    try:
        ...
        if(rows == 1): 
            sql = """Select number_of_copies,s.Game,Form_of_copy,s.price,s.name,s.location,g.Platform,p.Manufacturer
                from Store s
                join Game g
                on g.name = s.Game
                join Platform p 
                WHERE s.Game = ? and p.name IN (g.Platform)
            ...
            for row in rows:
                ...
                print("Runs on " + row[6] +" Manufacturer: " + row[7] + "\n")
            return row[1]
        else:
...
def check_funds(_conn,username):
    try:
        sql = """SELECT amount, location
                FROM Customer 
                WHERE name = ?"""
        args = [username]
        ...
        account = rows[0][0]
        location = rows[0][1]
        print("funds $"+ str(account))
        print("Loaction " + location)
        check = input("Add Funds Y or N: ")
        if(check in ['Y','y']):
            i_amount = input("Amount to Add: ")
            sql = """UPDATE Customer
                    SET amount = amount + ?
                    WHERE name = ?"""
            args = [i_amount,username]
...
def check_store(_conn,game,amount,type):
    try:
        sql = """SELECT EXISTS(SELECT 1 
                        FROM Store WHERE 
                        Game = ? and Form_of_copy = ?);"""
        args = [game,type]
        ...
        if(rows == 1):
            ...
            cur = _conn.cursor()
            cur.execute(sql,args)
            copies = int(cur.fetchone()[0])
            if(copies >= int(amount) ):
                return 1
            else:
                print("Not enought copies")
                return 0
        else:
            print("Form of copy does not exist")
            return 0
    except Error as e:
        print(e)
...
def Buy(_conn,username):
    ...
    if(game != None):
        amount = input("Amount: ")
        type = input("Form: ")
        check = check_store(_conn,game,amount,type)
        if(check == 1):
            try:
...
def publishing(_conn,game,developer):
    ...
    try:
        if(value == "s"):
            platform = platform_catalog(_conn,True)
            if(platform != 0):
                sql = """UPDATE Game
        ...
        with conn:
            if(user == "e"):
                ...
                if(action == 's'):
                    ...
                    platform = platform_catalog(conn,False)
                    if(platform != 0):
                        Buy_platform(conn,c_acc,platform)
                elif(action == 'c'):
                    check_funds(conn,c_acc)
                else:
                    print("command not recognized")
                status = input("\nStay logged in Y or N:")
                if(status in ['N','n']):
